...
- 稿件必须使用 Markdown 格式提交；
- 稿件开头必须包含规定元数据（不得删除 "）；
- 禁止提交作者隐私内容（如私人联系方式）、未获授权的稿件以及与校刊无关的文件。
...
- 关于链接
    - 不进入排版流程的文件（如 `README.md`，下同），应使用`[网站](链接)`的写法
    - 进入排版流程的文件（如正常稿件，下同），应使用`网站（链接）`的写法
